...
// Keep the shared session's event severity values host-neutral. Max's config
// frontend currently prints normal/debug events and reports Error events via
// object_error().
namespace {
constexpr int P2P_LOG_NORMAL = 0;
constexpr int P2P_LOG_DEBUG = 1;
constexpr int P2P_LOG_ERROR = 2;
}
...
bool P2PSession::setupWebRtc(const std::shared_ptr<P2PPeer> &peer) {
    ...
    peer->remote_description_set = false;
    peer->stun_warning_reported = false;
    log(P2P_LOG_DEBUG, "Max is polite=%d", peer->is_polite);
...
void P2PSession::decodeVideoFrame(const std::shared_ptr<P2PPeer> &peer,
    ...
    while (avcodec_receive_frame(peer->video_decoder, peer->video_frame) == 0) {
        ...
        if (!peer->video_decoded_logged) {
            ...
            log(P2P_LOG_NORMAL, "Decoded Jitter video frame: %dx%d", width, height);
        }
    }
}
#endif
